...
def is_valid(text):
    res = True
    for c in text:
        if ord(c) not in range(ord('А'), ord('Я') + 1):
            res = False
            break
    return res
...
def play(word):
    ...
    print('Добро пожаловать в игру - угадайка слов! Давайте же сыграем!')
    print(display_hangman(tries))
    print(*word_completion, sep='', end=' ') 
    print(f'{len(word)} Букв')

    while tries > 0 and '_' in word_completion:
        if guessed:
            break

        guess_try = input('Введите букву или слово целиком\n').upper()

        if guess_try == word or ''.join(word_completion) == word:
            guessed = True
            continue

        if not is_valid(guess_try):
            print('Необходимо ввести букву или слово на русской раскладке')
            continue

        if guess_try in guessed_letters:
            print('Такая буква уже была, попробуйте еще раз')
            continue
        elif guess_try in guessed_words:
            print('Такое слово уже было, попробуйте еще раз')
            continue

        if len(guess_try) == 1:
            guessed_letters.append(guess_try)
            if guess_try in word:
                for i in range(len(word)):
                    if word[i] == guess_try:
                        word_completion[i] = guess_try
                print(display_hangman(tries))
                print(f'Правильно! У Вас осталось {tries} попыток')
                print(*word_completion, sep='')
            else:
                tries -= 1
                print(display_hangman(tries))
                print(f'Не правильно. У Вас осталось {tries} попыток')
                print(*word_completion, sep='')
        else:
            guessed_words.append(guess_try)
            tries -= 1
            print(display_hangman(tries))
            print(f'Не правильно. У Вас осталось {tries} попыток')
            print(*word_completion, sep='')
...
while True:
    play(get_word())
    if not is_confirm('Вы хотите сыграть еще раз? (д - да, н - нет)'):
        break